...
export function canonicalInvoiceToRenderPayload(invoice, source = {}) {
  return {
    doc: { ...(source.doc ?? {}), number: invoice.invoiceNumber, date: invoice.issueDate, currency: invoice.currency, buyerReference: invoice.buyerReference, serviceDate: invoice.service.date, servicePeriodStart: invoice.service.periodStart, servicePeriodEnd: invoice.service.periodEnd, positions: invoice.lines.map((line) => ({ id: line.id, description: line.name, quantity: line.quantity, unit: line.unitCode, price: line.netUnitPrice, taxCategory: line.tax.category, taxRate: line.tax.rate })) },
    settings: { ...(source.settings ?? {}), companyName: invoice.seller.name, address: invoice.seller.addressLines.join("\n"), sellerStreet: invoice.seller.street, sellerHouseNumber: invoice.seller.houseNumber, sellerPostalCode: invoice.seller.postalCode, sellerCity: invoice.seller.city, sellerElectronicAddress: invoice.seller.electronicAddress, sellerTaxNumber: invoice.seller.taxNumber, sellerVatId: invoice.seller.vatId, currency: invoice.currency },
