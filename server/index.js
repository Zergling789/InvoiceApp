...
const getEmailIdempotencyRecord = async ({ key }) => {
  const supabase = requireSupabase();
  const { data, error } = await supabase
    .from("email_send_idempotency")
    .select("key, user_id, status, response_json")
    .eq("key", key)
    .maybeSingle();
  if (error) {
    console.warn("[email_idempotency] lookup_failed", { key, error: error.message });
  }
  return data ?? null;
};

const createEmailIdempotencyRecord = async ({ key, userId, requestId }) => {
  const supabase = requireSupabase();
...
const updateEmailIdempotencyRecord = async ({ key, status, responseJson, requestId }) => {
  const supabase = requireSupabase();
  const { error } = await supabase
    ...
    .eq("key", key);
  if (error) {
    console.warn("[email_idempotency] update_failed", { key, error: error.message });
  }
};
...
app.post("/api/pdf/link", requireAuth, async (req, res) => {
  try {
    ...
    res.json({ ok: true, url, requestId: req.requestId });
  } catch (err) {
    const status = err?.status || 500;
  ...
  async (req, res) => {
    try {
      ...
      if (!toList.length || toList.some((entry) => !isValidEmail(entry))) {
        return sendError(res, 400, "VALIDATION_ERROR", "Invalid recipient email.", req);
      }
      const ccList = parseEmailList(cc);
      if (ccList.some((entry) => !isValidEmail(entry))) {
        return sendError(res, 400, "VALIDATION_ERROR", "Invalid CC email.", req);
      }
      const bccList = parseEmailList(bcc);
      if (bccList.some((entry) => !isValidEmail(entry))) {
        return sendError(res, 400, "VALIDATION_ERROR", "Invalid BCC email.", req);
      }

      const subjectText = String(subject ?? "");
      if (subjectText.length > EMAIL_SUBJECT_MAX) {
        return sendError(res, 400, "VALIDATION_ERROR", "Subject too long.", req);
      }

      const messageText = String(message ?? "");
      if (messageText.length > EMAIL_MESSAGE_MAX) {
        return sendError(res, 400, "VALIDATION_ERROR", "Message too long.", req);
      }

      if (idempotencyKey) {
        const existing = await getEmailIdempotencyRecord({ key: idempotencyKey });
        if (existing?.user_id && existing.user_id !== userId) {
          return sendError(res, 409, "IDEMPOTENCY_KEY_REUSED", "Idempotency key already used.", req);
        }
        if (existing?.status === "sent") {
          console.info("[email_send_replay]", { requestId, idempotencyKey, userId });
          return res.status(200).json({ ...(existing.response_json ?? { ok: true }), requestId });
        }
        if (existing?.status === "pending") {
          return sendError(res, 409, "IDEMPOTENCY_IN_PROGRESS", "Email send already in progress.", req);
        }
        if (existing?.status === "failed") {
          await updateEmailIdempotencyRecord({
            key: idempotencyKey,
            status: "pending",
            requestId,
          });
        } else if (!existing) {
          const insertError = await createEmailIdempotencyRecord({
            key: idempotencyKey,
            userId,
            requestId,
          });
          if (insertError) {
            if (insertError.code === "23505") {
              const retryExisting = await getEmailIdempotencyRecord({ key: idempotencyKey });
              if (retryExisting?.status === "sent") {
                console.info("[email_send_replay]", { requestId, idempotencyKey, userId });
            ...
            console.warn("[email_idempotency] insert_failed", {
              requestId,
              idempotencyKey,
      ...
      console.info("[email_send_attempt]", { requestId, userId, docId, type, idempotencyKey });
      ...
      await Promise.all([
        ...
        console.warn("[email_post_send_update_failed]", { requestId, error: error?.message });
      });
      ...
      if (userToken) {
        ...
        if (markError) {
          console.warn("[email_mark_sent_failed]", {
            requestId,
            code: markError.code,
