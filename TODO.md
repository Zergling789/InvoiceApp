...
7. ⏳ **Noch offen:** Reale Gerätekontrollen mit tatsächlich geöffneter virtueller Tastatur auf iOS und Android. Die automatisierten Smartphone-, Rechnungs- und visuellen Hell-/Dunkelprüfungen laufen bereits im GitHub-CI-Testprojekt.
...
4. ✅ **Gemeinsame Oberflächen vereinheitlicht:** Bestätigungsdialoge, mobile Aktionsflächen, Toasts, Hinweise, Menüs und Kennzahlenkarten verwenden dieselben Radien, Oberflächen- und Statusfarben in Hell und Dunkel. Sichtbare technische Begriffe in Einstellungen und Dokumentstatus wurden entfernt. Der Audit hat zusätzlich einen falschen Angebotslink bei überfälligen Rechnungen behoben.
5. ✅ **Authentifizierter visueller Vergleich:** Die geschützten Kernseiten laufen mit kurzlebigem, mandantenisoliertem Testnutzer automatisiert auf Desktop und Smartphone im Hell- und Dunkelmodus. Der Test prüft horizontales Scrollen, globale Fehlerseiten, beschädigte UTF-8-Zeichen, Theme-Anwendung und mobile Navigation; Screenshots werden im CI sieben Tage als Artefakt aufbewahrt.

**Phase 7 umgesetzt:** Produktsprache, zentrale Statusfarben, Dialog- und Feedbackflächen sind vereinheitlicht und regressionsgetestet. Öffentliche und geschützte Kernseiten werden automatisiert in realistischen Desktop- und Smartphone-Viewports sowie im Hell- und Dunkelmodus geprüft. Reale iOS-/Android-Tastaturtests bleiben als gerätespezifischer Nachweis in Phase 3 offen.
...
14. ✅ **Einstellungen und Editor bedarfsgerecht teilen:** Die Standard-Einstellungsansicht lädt weder Absenderadressen noch Löschstatus und startet keinen Sekundentimer. E-Mail-, Konto- und Dokumentgestaltungs-Code wird erst beim Öffnen des jeweiligen Bereichs geladen. Erstellungskomponente und Aktivitätsverlauf liegen außerhalb des gemeinsamen Editor-Chunks. Der `SettingsView`-Chunk sinkt gzip von 9,79 auf 6,58 KiB, der gemeinsame `DocumentEditor`-Chunk von 19,53 auf 13,97 KiB.
15. ✅ **Authentifiziertes Mobilprofil:** Die Produktionsdateien werden mit einem kurzlebigen Testnutzer, 390 × 844 Pixeln, kaltem Cache und reproduzierbarer 4G-Drosselung geprüft. Feste Budgets begrenzen Zeit bis zur bedienbaren Rechnungserstellung, LCP, Layoutsprünge, lange Hauptthread-Aufgaben, JavaScript-Bytes, Request-Anzahl und die Reaktion des ersten Wizard-Schritts. Der Messbericht bleibt als CI-Artefakt nachvollziehbar.

**Phase 8 umgesetzt:** Das initiale JavaScript bleibt rund 39 Prozent kleiner. Dokument-, Kunden- und Projektübersichten sind cursor-paginiert; Suche und Statusfilter laufen vor dem Seitenlimit in der Datenbank, Projektkennzahlen werden separat und vollständig aggregiert. Große Einstellungs- und Editorbereiche laden selten genutzte Funktionen erst bei Bedarf. Bundle- und authentifizierte Mobilbudgets verhindern Regressionen bei Übertragungsgröße, Ladezeit und Interaktion.
...
3. ✅ **API-Negativmatrix:** Sämtliche authentifizierungspflichtigen HTTP-Endpunkte werden ohne Sitzung gegen denselben fail-closed Vertrag geprüft. Zusätzlich sichern echte HTTP-Tests Cross-Site-Blockade, ungültige öffentliche Empfänger-Tokens, Worker- und Webhook-Authentifizierung, fehlerhaftes JSON, Fehler-IDs und unbekannte API-Routen ab. Express-HTML-Fehler gelangen dadurch nicht mehr an den Browser.
4. ⏳ **Fortlaufende Aufgabe:** Jede weitere Roadmap-Verbesserung erhält passende Regressionstests. Vor Public Beta bleiben reale iOS-/Android-Tastaturtests, E-Mail-Provider-Zustelltests und ein isolierter Restore-Test nachzuholen.

**Phase 9 ist als laufendes Qualitäts-Gate aktiv:** Die lokale automatisierte Basis einschließlich der API-Negativmatrix ist breit vorhanden. Die verbleibenden Tests benötigen reale Geräte oder freigegebene externe Infrastruktur und können deshalb nicht allein durch lokale Mocks als abgeschlossen gelten.
