...
export default function RecipientDocumentPage() {
  ...
  const respond = async (response: "ACCEPTED" | "REJECTED") => { setSaving(true); try { await respondToOffer(token, response); setData(current => current ? { ...current, response } : current); } catch (cause) { setError(cause instanceof Error ? cause.message : "Antwort konnte nicht gespeichert werden."); } finally { setSaving(false); } };
  ...
  return <main className="min-h-screen bg-[var(--app-bg)] p-5 text-[var(--app-text)] sm:p-10"><article className="app-card mx-auto max-w-3xl space-y-7 p-6 sm:p-10">
    ...
    {data.type === "offer" && <section className="border-t border-[var(--app-border)] pt-6"><h2 className="font-semibold">Wie möchten Sie antworten?</h2><p className="mt-2 text-sm text-[var(--app-muted)]">Bitte bestätigen Sie Ihre Entscheidung über eine der folgenden Schaltflächen.</p>{data.response ? <div className="mt-4 rounded-xl bg-emerald-500/10 p-4 font-semibold">Antwort gespeichert: {data.response === "ACCEPTED" ? "Angenommen" : "Abgelehnt"}</div> : <div className="mt-4 flex flex-wrap gap-3"><AppButton disabled={saving} onClick={() => void respond("ACCEPTED")}>{saving ? "Antwort wird gespeichert…" : "Angebot annehmen"}</AppButton><AppButton variant="secondary" disabled={saving} onClick={() => void respond("REJECTED")}>Angebot ablehnen</AppButton></div>}{error && <p role="alert" className="mt-3 text-sm text-red-600">{error}</p>}</section>}
  </article></main>;
}
