...
export default function RecipientDocumentPage() {
  ...
  return <main className="min-h-screen bg-[var(--app-bg)] p-5 text-[var(--app-text)] sm:p-10"><article className="app-card mx-auto max-w-3xl space-y-7 p-6 sm:p-10">
    ...
    {data.type === "offer" && <section className="border-t border-[var(--app-border)] pt-6"><h2 className="font-semibold">Wie möchten Sie antworten?</h2><p className="mt-2 text-sm text-[var(--app-muted)]">Bitte bestätigen Sie Ihre Entscheidung über eine der folgenden Schaltflächen.</p>{data.response ? <div className="mt-4 rounded-xl bg-emerald-500/10 p-4"><div className="font-semibold">Antwort gespeichert: {data.response === "ACCEPTED" ? "Angenommen" : "Abgelehnt"}</div>{data.responseReason && <p className="mt-2 whitespace-pre-line text-sm">Begründung: {data.responseReason}</p>}</div> : <div className="mt-4 space-y-3"><div className="flex flex-wrap gap-3"><AppButton disabled={saving} onClick={() => void respond("ACCEPTED")}>{saving ? "Antwort wird gespeichert…" : "Angebot annehmen"}</AppButton><AppButton variant="secondary" disabled={saving} onClick={() => void respond("REJECTED")}>Angebot ablehnen</AppButton></div><label htmlFor="rejection-reason" className="block max-w-xl text-sm"><span className="font-medium">Optionale Begründung bei Ablehnung</span><textarea id="rejection-reason" className="app-input mt-2 min-h-20 w-full resize-y" maxLength={500} value={rejectionReason} onChange={(event) => setRejectionReason(event.target.value)} placeholder="Zum Beispiel: Budget aktuell nicht verfügbar" /><span className="mt-1 block text-right text-xs text-[var(--app-muted)]">{rejectionReason.length}/500</span></label></div>}{error && <p role="alert" className="mt-3 text-sm text-red-600">{error}</p>}</section>}
  </article></main>;
}
