    ...
    <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_300px]">
      <main className="min-w-0 space-y-4">
        ...
        <Section title="Abrechnung" summary={[value.paymentTermsDays != null && `${value.paymentTermsDays} Tage`, value.currency, value.defaultVatRate != null && `${value.defaultVatRate} %`].filter(Boolean).join(" · ") || "Noch nicht konfiguriert"}>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{textField("Rechnungs-E-Mail", "invoiceEmail", { type: "email" })}<label className="text-sm font-medium">Zahlungsziel<input type="number" min="0" max="365" className={inputClass} value={value.paymentTermsDays ?? ""} onChange={(event) => onChange({ ...value, paymentTermsDays: event.target.value === "" ? null : Number(event.target.value) })} /></label><label className="text-sm font-medium">Währung<select className={inputClass} value={value.currency ?? ""} onChange={(event) => onChange({ ...value, currency: event.target.value })}><option value="">App-Standard</option>{["EUR", "USD", "CHF", "GBP"].map((currency) => <option key={currency}>{currency}</option>)}</select></label><label className="text-sm font-medium">Standard-MwSt.<input type="number" min="0" max="100" className={inputClass} value={value.defaultVatRate ?? ""} onChange={(event) => onChange({ ...value, defaultVatRate: event.target.value === "" ? null : Number(event.target.value) })} /></label><label className="text-sm font-medium">Versandweg<select className={inputClass} value={value.preferredDeliveryMethod ?? "email"} onChange={(event) => onChange({ ...value, preferredDeliveryMethod: event.target.value as Client["preferredDeliveryMethod"] })}><option value="email">E-Mail</option><option value="download">Download</option><option value="post">Post</option></select></label><label className="text-sm font-medium">Sprache<select className={inputClass} value={value.preferredLanguage ?? "de"} onChange={(event) => onChange({ ...value, preferredLanguage: event.target.value })}><option value="de">Deutsch</option><option value="en">Englisch</option></select></label><label className="text-sm font-medium sm:col-span-2 lg:col-span-3">Abweichende Rechnungsadresse<textarea rows={3} className={inputClass} value={value.billingAddress ?? ""} onChange={(event) => onChange({ ...value, billingAddress: event.target.value })} /></label></div>
        </Section>

        <Section title="Organisation" summary={[value.tags?.length && `${value.tags.length} Tags`, value.source && `Quelle: ${value.source}`].filter(Boolean).join(" · ") || "Noch nicht konfiguriert"}><div className="grid gap-4 sm:grid-cols-2">{textField("Kontaktquelle", "source")}<label className="text-sm font-medium">Tags<input className={inputClass} value={(value.tags ?? []).join(", ")} placeholder="VIP, Empfehlung, Agentur" onChange={(event) => onChange({ ...value, tags: event.target.value.split(",").map((tag) => tag.trim()).filter(Boolean) })} /></label><label className="text-sm font-medium">Letzter Kontakt<input type="datetime-local" className={inputClass} value={value.lastContactAt?.slice(0, 16) ?? ""} onChange={(event) => onChange({ ...value, lastContactAt: event.target.value || null })} /></label><label className="text-sm font-medium">Nächstes Follow-up<input type="datetime-local" className={inputClass} value={value.nextFollowUpAt?.slice(0, 16) ?? ""} onChange={(event) => onChange({ ...value, nextFollowUpAt: event.target.value || null })} /></label></div></Section>
