...
import { dbUpsertOffer } from "@/db/offersDb";
import { dbUpsertInvoice } from "@/db/invoicesDb";
...
export function DocumentEditor({
  ...
  const totals = useMemo(() => {
    const subtotal = (formData.positions ?? []).reduce(
      (acc, p) => acc + toNumberOrZero(p.quantity) * toNumberOrZero(p.price),
      0
    );
    const tax = subtotal * (toNumberOrZero(formData.vatRate) / 100);
          ...
          <div className="no-print flex justify-between mb-8 p-4 bg-gray-100 rounded-lg">
            ...
            <div className="flex gap-2">
              <AppButton onClick={() => window.print()}>
                <FileDown size={16} /> Drucken / PDF speichern
              </AppButton>

              <a
                href={`mailto:${client?.email ?? ""}?subject=${encodeURIComponent(
                  `${isInvoice ? "Rechnung" : "Angebot"} ${formData.number}`
                )}&body=${encodeURIComponent("Anbei erhalten Sie das Dokument.")}`}
