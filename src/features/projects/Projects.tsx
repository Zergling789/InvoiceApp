...
import { useProjectMetrics, useProjectPages } from "@/app/projects/projectQueries";
...
export default function Projects() {
  ...
  const matchingClientIds = useMemo(() => {
    ...
    const normalizedQuery = debouncedQuery.toLocaleLowerCase("de-DE");
    return clients
      .filter((client) =>
        `${getClientDisplayName(client)} ${getClientPersonName(client)}`
  ...
  }, [location.hash, location.pathname, location.search, location.state, navigate, refreshClients, refreshMetrics, refreshProjects]);
    ...
    <div className="space-y-6">
      ...
      {!metricsLoading && !metricsError && (
        <div className="grid gap-3 sm:grid-cols-2">
          <AppCard className="p-4"><div className="text-xs font-semibold uppercase tracking-wide text-[var(--app-muted)]">Laufende Projekte</div><div className="mt-2 text-2xl font-semibold">{metrics.activeCount}</div></AppCard>
          <AppCard className="p-4"><div className="text-xs font-semibold uppercase tracking-wide text-[var(--app-muted)]">Geplanter Auftragswert</div><div className="mt-2 text-2xl font-semibold">{formatMoney(metrics.plannedValue, "EUR", "de-DE")}</div></AppCard>
        </div>
      )}

      {error && <div role="alert" className="flex flex-col gap-3 rounded-2xl border border-red-500/30 bg-red-500/10 p-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="font-semibold text-red-700 dark:text-red-300">Projekte konnten nicht geladen werden</div><div className="mt-1 text-sm text-red-700 dark:text-red-200">Bitte prüfe deine Verbindung und versuche es erneut.</div></div><AppButton variant="secondary" onClick={() => void Promise.all([refreshProjects(), refreshClients(), refreshMetrics()])}>Erneut versuchen</AppButton></div>}

      {!loading && (projects.length > 0 || query || statusFilter !== "all") && (
      ...
      ) : projects.length === 0 && !error && !debouncedQuery && statusFilter === "all" ? (
      ...
      {!loading && !error && hasMore && (
        <div className="flex flex-col items-center gap-2">
          {loadMoreError && <p role="alert" className="text-sm text-red-600">Weitere Projekte konnten nicht geladen werden.</p>}
          <AppButton variant="secondary" disabled={loadingMore} onClick={() => void loadMore()}>
            {loadingMore ? "Weitere Projekte werden geladen..." : "Weitere Projekte laden"}
          </AppButton>
        </div>
      )}
    </div>
