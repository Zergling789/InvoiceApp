...
declare
  ...
  rate numeric;
begin
  if new.status = 'DRAFT' or (tg_op = 'UPDATE' and old.status is not distinct from new.status) then
    return new;
  end if;
  ...
  loop
    ...
    if category not in ('STANDARD', 'REDUCED', 'ZERO', 'EXEMPT', 'SMALL_BUSINESS') then raise exception 'UNSUPPORTED_TAX_CASE' using errcode = 'P0001'; end if;
    if category in ('ZERO', 'EXEMPT', 'SMALL_BUSINESS') and coalesce(rate, 0) <> 0 then raise exception 'POSITION_TAX_INVALID' using errcode = 'P0001'; end if;
